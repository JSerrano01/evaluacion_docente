...
$query_eval_estudiantes = "SELECT  GRUPO, COUNT(GRUPO) as COUNT_ENC, ENCUESTA, ROUND(((PREGUNTA1 + PREGUNTA4 + PREGUNTA5 + PREGUNTA6 + PREGUNTA7+ PREGUNTA8+ PREGUNTA9+ PREGUNTA10+ PREGUNTA11+ PREGUNTA12) / 10),1) AS gestion_asig, ROUND(((PREGUNTA12 + PREGUNTA13 + PREGUNTA14 + PREGUNTA15) / 4) ,1) AS ambiente_asig, ROUND(((PREGUNTA16 + PREGUNTA17 + PREGUNTA18 + PREGUNTA19 + PREGUNTA20) / 5),1) AS motivacion_asig, ROUND(((PREGUNTA21 + PREGUNTA22 + PREGUNTA23 + PREGUNTA24 + PREGUNTA25) / 5),1) AS evaluacion_asig, ROUND(((PREGUNTA26 + PREGUNTA27 + PREGUNTA28 + PREGUNTA29 + PREGUNTA30 + PREGUNTA31 + PREGUNTA32 + PREGUNTA33 + PREGUNTA34 + PREGUNTA35 + PREGUNTA36 + PREGUNTA37 + PREGUNTA38 + PREGUNTA39) / 14),1) AS comunicacion_asig FROM e_estud WHERE DOCUMENTO_DOCENTE = :documento GROUP BY GRUPO";
...
reset($data_estudiantes2); // Coloca el puntero interno al inicio del arreglo
while (current($data_estudiantes2)) {
    $estudiante = current($data_estudiantes2);
    ...
    next($data_estudiantes2); // Avanza el puntero interno al siguiente elemento
}

// Mostrar las respuestas por grupo
reset($respuestas_por_grupo); // Coloca el puntero interno al inicio del arreglo
while (current($respuestas_por_grupo)) {
    $grupo = key($respuestas_por_grupo);
    $respuestas = current($respuestas_por_grupo);
    $pdf->Cell(30, 10, "Grupo:" . $grupo, 0, 0, 'L');
    $pdf->Ln();
    reset($respuestas); // Coloca el puntero interno de $respuestas al inicio del arreglo
    while (current($respuestas)) {
        $respuesta = current($respuestas);
        $pdf->Row(array(" - ", $respuesta));
        next($respuestas); // Avanza el puntero interno de $respuestas al siguiente elemento
    }
    next($respuestas_por_grupo); // Avanza el puntero interno de $respuestas_por_grupo al siguiente elemento
    $pdf->Ln();
}
